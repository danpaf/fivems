...
static InitFunction initFunction([]()
{
	fx::ServerInstanceBase::OnServerCreate.Connect([](fx::ServerInstanceBase* instance)
	{
		...
		instance->GetComponent<fx::ClientMethodRegistry>()->AddHandler("initConnect", [=](const std::map<std::string, std::string>& postMap, const fwRefContainer<net::HttpRequest>& request, const std::function<void(const json&)>& cb)
		{
			...
			// SolarProject: own auth system — no Cfx.re tickets required
			// Players authenticate by GUID + IP, optional solarToken for future auth server
			{
				auto solarTokenIt = postMap.find("solarToken");
				if (solarTokenIt != postMap.end())
				{
					console::DPrintf("server", "SolarProject: player '%s' (guid %s) connected with auth token\n", name, guid);
				}
				else
				{
					console::DPrintf("server", "SolarProject: player '%s' (guid %s) connected via open auth\n", name, guid);
				}

				// Assign a solar: identifier based on the client GUID
				// This provides a stable per-machine identity without external auth
			}
			...
			// SolarProject: always allow game name match — no Cfx.re ticket needed
			gameNameMatch = true;
